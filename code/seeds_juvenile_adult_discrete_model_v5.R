...
Kl_lin_hi <- bl_fun(light = light_val, poly = 1, bio = pop_b)
Kl_lin_lo <- bl_fun(light = light_val, poly = 1, bio = pop_b/2)
...
Kl_up_hi <- bl_fun(light = light_val, poly = 2, h = max_light/2, j = 300, bio = pop_b)
Kl_down_hi <- bl_fun(light = light_val, poly = 2, h = max_light/2, j = 200, bio = pop_b)
Kl_up_lo <- bl_fun(light = light_val, poly = 2, h = max_light/2, j = 200, bio = pop_b/2)
Kl_down_lo <- bl_fun(light = light_val, poly = 2, h = max_light/2, j = 100, bio = pop_b/2)
...
Kl_curves <- tibble(light = rep(light_val, 6),
                    biomass = c(Kl_lin_hi, Kl_lin_lo, Kl_up_hi, Kl_down_hi, Kl_up_lo, Kl_down_lo),
                    approach = rep(c(rep("traditional", 2), rep("multiple points", 4)), each = light_n),
                    stress_change = rep(c("none", "lower slope", "none", "lower peak", "lower slope", "lower slope\nand peak"), each = light_n)) %>%
  mutate(approach = fct_relevel(approach, "traditional"),
         stress_change = fct_relevel(stress_change, "none", "lower slope", "lower peak"))
...
# figure
pdf("output/model_v5/light_biomass_function_shapes.pdf")
ggplot(bl_curves, aes(light, biomass, color = stress_change)) +
  ...
  scale_fill_manual(values = c("#286C7B", "#A74A44", rep("white", 4)), guide = "none") +
  xlab(expression(paste("Light availability (", mu, "moles ", m^-2, " ", sec^-1, ")", sep = ""))) +
  ylab("Biomass (g)") +
  fig_theme +
  theme(legend.position = c(0.16, 0.8))

ggplot(Kl_curves, aes(light, biomass, color = stress_change)) +
  geom_line(aes(linetype = approach)) +
  geom_point(data = Kl_points, size = 3, aes(shape = stress_change, color = stress_change, fill = app_stress)) +
  scale_linetype_manual(values = c("solid", "dashed"), name = "Approach") +
  scale_shape_manual(values = c(21, 21, 22, 22), name = "Stress effect") +
  scale_color_manual(values = c("#286C7B", "#A74A44", "#A2CEDE", "#E93F33"), name = "Stress effect") +
  scale_fill_manual(values = c("#286C7B", "#A74A44", rep("white", 4)), guide = "none") +
  xlab(expression(paste("Light availability (", mu, "moles ", m^-2, " ", sec^-1, ")", sep = ""))) +
  ylab("Population-scale biomass (g)") +
  fig_theme +
  theme(legend.position = c(0.16, 0.8))
dev.off()
...
# is the same as V4 for individual biomass, so did not re-run
...
# 
# # figure
# pdf("output/model_v5/biomass_espi_separate.pdf")
...
# dev.off()
# 
# pdf("output/model_v5/biomass_espi.pdf")
...
for(i in 1:nrow(res_curve)){
  ...
  # run model
  temp_mod <- sim_fun2(bio = res_curve$biomass[i],
                       K = res_curve$pop_biomass[i])
...
pdf("output/model_v5/trees_change.pdf")
ggplot(res_curve2, aes(light, trees, color = stress_change, linetype = approach)) +
  geom_line() +
  scale_color_manual(values = c("#286C7B", "#A74A44", "#A2CEDE", "#E93F33"), name = "Stress effect") +
  scale_linetype_manual(values = c("solid", "dashed"), name = "Approach") +
  xlab(expression(paste("Light availability (", mu, "moles ", m^-2, " ", sec^-1, ")", sep = ""))) +
